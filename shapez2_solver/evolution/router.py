...
class BeltRouter:
    ...
    def get_neighbors(self, x: int, y: int, floor: int) -> List[Tuple[int, int, int, Rotation]]:
        ...
        for dx, dy, df, direction in directions:
            ...
            if self.is_valid(nx, ny, nf):
                neighbors.append((nx, ny, nf, direction))

        # Floor changes (lifts)
        if floor < self.num_floors - 1 and self.is_valid(x, y, floor + 1):
            neighbors.append((x, y, floor + 1, Rotation.EAST))  # Lift up
        if floor > 0 and self.is_valid(x, y, floor - 1):
            neighbors.append((x, y, floor - 1, Rotation.EAST))  # Lift down
    ...
    def find_path(self, start: Tuple[int, int, int], goal: Tuple[int, int, int]) -> Optional[List[Tuple[int, int, int]]]:
        ...
        while open_set:
            ...
            for nx, ny, nf, _ in self.get_neighbors(current[0], current[1], current[2]):
                neighbor = (nx, ny, nf)
    ...
    def path_to_belts(self, path: List[Tuple[int, int, int]]) -> List[Tuple[int, int, int, BuildingType, Rotation]]:
        ...
        for i in range(len(path) - 1):
            ...
            # Determine belt type and rotation
            if dz > 0:
            ...
            belts.append((curr[0], curr[1], curr[2], belt_type, rotation))
